...
public class MainActivity extends AppCompatActivity {
    ...
    public void onClickSignIn(){
        ...
        // confirm if the username is written
        if(sid.length()==0){
        ...
        // confirm if the password is written
        if(spsw.length()==0){
        ...
        // check if the password is right by calling matchPassword function and returns results
        // res = 0: successful
        // res = 1: I/O error is caught
        // res = 2: successful, but the server gives safety instructions (like login too many times in a day)
        // res = 3: match failed, wrong credentials
        int res = matchpassword(sid, spsw);

        //send hint messages via toasts
        Toast tot;

        switch (res){
            case 2:
                // if web page gives safety instructions, login is successful
                // but information page may not work
                // this issue needs to inform the user
                tot = Toast.makeText(this, "Server is busy.\nSome functions may not work.", Toast.LENGTH_LONG);
                tot.show();
            case 0:
                // if login is successful, system will enter the main frame of this application
                // the username written in this page will be transmitted into the main frame
                ...
                break;
            case 1:
                // if an exception of I/O is caught, system will inform the user
                tot = Toast.makeText(this, "I/O Error! Please contact admin.", Toast.LENGTH_LONG);
                tot.show();
                break;
            case 3:
                // if the login is failed, it is because of the mismatch between username and password
                // the system should inform the user the name or the password may be wrong
    ...
    private int matchpassword(String id, String psw){
        ...
        RequestBody payload = new FormBody.Builder()
                ...
                .add("__VIEWSTATEGENERATOR","CAA0A5A7") //static
                .add("aerererdsdxcxdfgfg","") //static
                .add("efdfdfuuyyuuckjg", md5psw) //ciphered password
                .add("pcInfo","") //static
                .add("txt_dsdfdfgfouyy", "") //static, originally used for ciphering password
                .add("txt_dsdsdsdjkjkjc", id) // student cquID
                .add("txt_ysdsdsdskgf","") //static
                .add("typeName","") //static
                .build();
        Request login = new Request.Builder()
                .url("http://jxgl.cqu.edu.cn/_data/index_login.aspx") //request page
                .post(payload)
                ...
                .build();
        //getting information via foot.aspx to check if the login is successful
        Request parsefoot = new Request.Builder()
                .url("http://jxgl.cqu.edu.cn/PUB/foot.aspx") //test page
                .get()
        ...
        Response logres;
        String infores;
        try{//sending post requests and getting feedbacks
        ...
        //the correct response should be "[cquID]student name"
        //if login is failed, the response is "[]"
        //if the safety instructions are given, it will have no brackets in response
